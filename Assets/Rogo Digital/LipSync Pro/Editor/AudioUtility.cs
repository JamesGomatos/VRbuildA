...
namespace RogoDigital {
	public static class AudioUtility {
		...
		public static void DrawWaveForm (AudioClip clip, int channel, Rect position) {
				...
				if (clip == storedClip) {
					...
					AudioCurveRendering.DrawMinMaxFilledCurve(
						...
						delegate (float x, out Color col, out float minValue, out float maxValue) {
							...
							int offset1 = Mathf.Clamp(((i * numChannels + channel) * 2), 0, minMaxData.Length - 2);
			...
			storedClip = clip;
			minMaxData = AudioUtil_GetMinMaxData.Invoke(null, new object[] { importer }) as float[]; if (minMaxData == null) return;

			DrawWaveForm(clip, channel, position);
		}
#else
